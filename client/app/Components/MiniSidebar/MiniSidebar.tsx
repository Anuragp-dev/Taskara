        ...
        <ul className='flex flex-col gap-10'>
          {navItems.map((item, index) => (
            ...
            <li key={index} className='relative group'>
              ...
              {/* hover tooltip */}
              <span className='u-triangle absolute top-[20%] translate-y-[-50%] left-8 text-xs pointer-events-none text-white bg-[#3aafae] bg-2 px-2 py-1 rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300' >
                {item.title}
              </span>
